    ...
    <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="text-xs text-slate-500">Showing {users.length} result(s)</div>
        </div>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">Joined: {u.joinedAt}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-slate-600">{u.email}</td>
                <td className="px-4 py-3">
                  <Badge variant="fuchsia">{u.role === "TALENT" ? "Talent" : "Employer"}</Badge>
                </td>
                <td className="px-4 py-3">
                  {u.status === "ACTIVE" && <Badge variant="green">Active</Badge>}
                  {u.status === "SUSPENDED" && <Badge variant="yellow">Suspended</Badge>}
                  {u.status === "BLOCKED" && <Badge variant="red">Blocked</Badge>}
                </td>
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-2">
                    <button className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200">
                      Edit
                    </button>
                    <button className="rounded-xl bg-fuchsia-800 px-3 py-1.5 text-xs font-bold text-white hover:bg-fuchsia-700">
                      View
                    </button>
                    <button className="rounded-xl bg-rose-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-rose-500">
                      Delete
                    </button>
                  </div>
                </td>
